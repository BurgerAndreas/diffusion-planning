...
def clone_body(body, links=None, collision=True, visual=True, client=None):
    ...
    new_body = p.createMultiBody(
        ...
        baseCollisionShapeIndex=(
            clone_collision_shape(body, base_link, client) if collision else NULL_ID
        ),
        baseVisualShapeIndex=(
            clone_visual_shape(body, base_link, client) if visual else NULL_ID
        ),
...
def get_ray_from_to(mouseX, mouseY, farPlane=10000):
    ...
    width, height, _, _, _, camForward, horizon, vertical, _, _, dist, camTarget = (
